...
- (void)windowDidResize:(NSNotification *)notification
{
    ...
    {
        NSRect rect = NSRectFromCGRect(CGRectMake(0, 20, 280, 100));
        ConfigView * aConfigView = [[ConfigView alloc] initWithFrame:rect];
        self.configView = aConfigView;
        [self.backgroundView addSubview:self.configView];
        [aConfigView release];
    }
    [self.configView setHidden:YES];
}
...
- (void)runSearch
{
    AppDelegate * parent = (AppDelegate*)[NSApplication sharedApplication].delegate;
    
    NSString *searchString = [self.searchField stringValue];
    
    if ([searchString length] > 0)
    {
        [parent search:searchString];
    }
    if(parent.currentEntry != nil)
    {
        NSString * title = [parent.currentEntry title];
        NSString * name = [parent.currentEntry username];
        [self.textField setStringValue:[NSString stringWithFormat:@"%@ | %@", title, name]];
    }
...
{
    [self.searchField setHidden:!self.searchField.isHidden];
    [self.textField setHidden:!self.textField.isHidden];
    [self.configView setHidden:!self.searchField.isHidden];
}
